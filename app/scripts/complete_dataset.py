...
def save_result(synthesized, case_dir, missing_modality, output_dir):
    """Save ONLY the synthesized modality directly to output_dir (no subfolders)."""
    case_name = os.path.basename(case_dir)
    
    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Save ONLY the synthesized modality directly to output_dir
    output_filename = f"{case_name}-{missing_modality}.nii.gz"
    output_path = os.path.join(output_dir, output_filename)
    
    # Get reference for header/affine from an existing file in the case
